...
interface Farm {
  ...
  const BlockchainDataDisplay = () => {
    ...
    // Contract addresses
    const FARM_REGISTRY_ADDRESS = "0x2263d5D6344f4ce0c3B6af9A9cD73027baBb82C8";
    const PRODUCT_REGISTRY_ADDRESS = "0x903A7Dc88472e06DEe064186CbB037E4C79cfBEF";
  ...
  const getStatusString = (status: number) => {
    const statusMap = ['Created', 'InTransit', 'AtDistributor', 'AtRetailer', 'Sold'];
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {farms.map((farm, index) => (
              <Card key={index} className="p-4">
                ...
                <p className="text-sm">Owner: {farm.owner.slice(0, 6)}...{farm.owner.slice(-4)}</p>
                <p className="text-sm">Status: {farm.isActive ? 'Active' : 'Inactive'}</p>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {products.map((product, index) => (
              <Card key={index} className="p-4">
                <h3 className="font-bold mb-2">{product.productType}</h3>
                <p className="text-sm">Farm ID: {product.farmId.toString()}</p>
                <p className="text-sm">Quantity: {product.quantity.toString()} {product.unit}</p>
                <p className="text-sm">Status: {getStatusString(product.status)}</p>
                <p className="text-sm">Harvest Date: {new Date(Number(product.harvestDate) * 1000).toLocaleDateString()}</p>
                ...
                <p className="text-sm mt-2">Additional Info: {product.additionalInfo}</p>
              </Card>
            ))}
          </div>
        </CardContent>
